...
@ApplicationScoped
public class PublicBlobLinkCache {
	private static final String MAP_NAME = "public-blob-links";
	private final ClusteredMap<String, PublicBlobLinkEntry> cache;

	@Inject
	public PublicBlobLinkCache(ClusterMapService clusterMapService) {
		this.cache = clusterMapService.getMap(MAP_NAME, String.class, PublicBlobLinkEntry.class);
	}

	public Optional<PublicBlobLink> getOrCreate(
			UUID workspaceId,
			String hash,
			Duration ttl,
		...
		if (existing != null && existing.expiresAt().isAfter(now.plusSeconds(60))) {
		...
		created.ifPresent(link -> cache.put(key, new PublicBlobLinkEntry(link.uri(), link.expiresAt())));
		return created;
	}
