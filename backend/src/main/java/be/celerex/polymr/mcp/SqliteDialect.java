...
@ApplicationScoped
public class SqliteDialect implements CustomDialect {
	@Override
	public boolean supports(String dbKind) {
		return dbKind != null && dbKind.toLowerCase(Locale.ROOT).contains("sqlite");
	}
	...
	public Query createPagePathSearchQuery(
		...
		String sql = "select " + selectClause + " from ("
			...
			+ " and (:pageType is null or upper(cast(p.page_type as text)) = :pageType)"
			+ " ) page_paths"
			+ " where (:root is null or path = :root or path like :rootPrefix)"
