...
@ApplicationScoped
public class PostgresqlDialect implements CustomDialect {
	@Override
	...
	public Query createScriptPathSearchQuery(EntityManager entityManager, UUID workspaceId, String root, String pattern, boolean countOnly) {
		...
		String sql = "select " + selectClause + " from ("
			+ " select case"
			+ " when s.namespace is null or btrim(s.namespace) = '' then coalesce(nullif(btrim(s.name), "
			+ "''), s.slug) || '.groovy'"
			+ " else s.namespace || '/' || coalesce(nullif(btrim(s.name), ''), s.slug) || '.groovy'"
			+ " end as path"
			+ " from scripts s"
			+ " where s.workspace_id = :workspaceId and s.disabled = false"
			+ " ) script_paths"
			+ " where (cast(:root as text) is null or path = cast(:root as text) or path like "
			+ "cast(:rootPrefix as text))"
			+ " and (cast(:pattern as text) is null or lower(path) like cast(:patternLike as text))"
	...
	public Query createPagePathSearchQuery(
		...
		String sql = "select " + selectClause + " from ("
			+ " select case"
			+ " when p.namespace is null or btrim(p.namespace) = '' then coalesce(nullif(btrim(p.name), "
			+ "''), p.slug) || '.vue'"
			+ " else p.namespace || '/' || coalesce(nullif(btrim(p.name), ''), p.slug) || '.vue'"
			+ " end as path"
			+ " from sfc_pages p"
			+ " where p.workspace_id = :workspaceId and p.disabled = false"
			+ " and (cast(:pageType as text) is null or upper(cast(p.page_type as text)) = cast(:pageType "
			+ "as text))"
			+ " ) page_paths"
			+ " where (cast(:root as text) is null or path = cast(:root as text) or path like "
			+ "cast(:rootPrefix as text))"
			+ " and (cast(:pattern as text) is null or path ~* cast(:pattern as text))"
			+ (countOnly ? "" : " order by lower(path)");
		Query query = entityManager.createNativeQuery(sql);
		query.setParameter("workspaceId", workspaceId);
