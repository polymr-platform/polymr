...
@ApplicationScoped
public class DuckdbDialect implements CustomDialect {
	@Override
	public boolean supports(String dbKind) {
		return dbKind != null && dbKind.toLowerCase(Locale.ROOT).contains("duckdb");
	}
	...
	public Query createScriptContentSearchQuery(
		...
		String sql = "select path, source from ("
			...
			+ " ) searchable_scripts"
			+ " where (:root is null or path = :root or path like :rootPrefix)"
			+ " and regexp_matches(source, :pattern, 'c')"
			+ " order by regexp_count(source, :pattern, 'c') desc, lower(path)"
	...
	public Query createPagePathSearchQuery(
		...
		String sql = "select " + selectClause + " from ("
			...
			+ " and (:pageType is null or upper(cast(p.page_type as varchar)) = :pageType)"
			+ " ) page_paths"
			+ " where (:root is null or path = :root or path like :rootPrefix)"
			+ " and (:pattern is null or regexp_matches(path, :pattern, 'i'))"
			+ (countOnly ? "" : " order by lower(path)");
		Query query = entityManager.createNativeQuery(sql);
		query.setParameter("workspaceId", workspaceId);
