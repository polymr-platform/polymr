...
@ApplicationScoped
public class ScriptApiDefinitionService {
	private static final String BASE_PATH = "/script-api/";
	...
	public String loadDefinition(String name) {
		...
		try (InputStream stream = getClass().getResourceAsStream(BASE_PATH + file)) {
			...
			org.jboss.logging.Logger
				.getLogger(ScriptApiDefinitionService.class)
				.debugf(ex, "Failed to load script API definition %s", name);
		}
		return null;
	}
}
