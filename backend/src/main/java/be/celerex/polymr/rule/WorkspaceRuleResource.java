...
@Path("/api/tenants/{tenantId}/workspaces/{workspaceId}/rules")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class WorkspaceRuleResource {
	@Inject
	...
	public List<RuleResponse> list(
		...
		return entityManager.createQuery(
				"select new be.celerex.polymr.rule.dto.RuleResponse(r.id, r.name, r.content, "
					...
					+ "where r.scope = :scope and r.workspace.id = :workspaceId "
	...
	public RuleResponse create(
		...
		Rule rule = new Rule();
		rule.scope = RuleScope.WORKSPACE;
	...
	public RuleResponse update(
		...
		if (rule == null
				|| rule.scope != RuleScope.WORKSPACE
				|| rule.workspace == null
				|| !rule.workspace.id.equals(workspaceId)) {
