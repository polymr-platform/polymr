...
@Path("/api/tenants/{tenantId}/workspaces/{workspaceId}/skills")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class WorkspaceSkillResource {
	@Inject
	...
	public List<SkillResponse> list(
		...
		return entityManager.createQuery(
				"select new be.celerex.polymr.skill.dto.SkillResponse(s.id, s.workspace.id, s.name, "
					+ "s.trigger, s.description,"
					+ " s.alwaysIncluded, s.promptText) from Skill s where s.workspace.id = :workspaceId",
				SkillResponse.class
			)
	...
	public SkillResponse create(
		...
		Workspace workspace = requireWorkspace(tenantId, workspaceId);
		validate(request);
		if (request.workspace_id() != null && !workspaceId.equals(request.workspace_id())) {
			throw new WebApplicationException("Workspace mismatch", Response.Status.BAD_REQUEST);
		}
	...
	public SkillResponse update(
		...
		if (skill == null
				|| !skill.tenant.id.equals(tenantId)
				|| skill.workspace == null
				|| !skill.workspace.id.equals(workspaceId)) {
