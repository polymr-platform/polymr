...
async function writeBlob(url, blob) {
  if (!url.match("https://huggingface.co") && !url.startsWith("custom://")) return;
  try {
...
const _TtsSession = class _TtsSession {
  ...
  async init() {
    ...
    if (typeof window !== "undefined" && window.ort) {
      __privateSet(this, _ort, window.ort);
    } else {
      throw new Error("ONNX runtime not available on window.ort");
    }
    ...
    const configUrl = isCustom ? `custom://${path}.json` : `${HF_BASE}/${path}.json`;
    const modelUrl = isCustom ? `custom://${path}.onnx` : `${HF_BASE}/${path}`;
    const modelConfigBlob = await getBlob(configUrl);
    __privateSet(this, _modelConfig, JSON.parse(await modelConfigBlob.text()));
    const modelBlob = await getBlob(modelUrl, __privateGet(this, _progressCallback));
